...
pub unsafe fn holding(mut lk: *mut Spinlock) -> libc::c_int {
    ...
    push_off();
    r = ((*lk).locked != 0 && (*lk).cpu == proc::mycpu()) as libc::c_int;
    pop_off();
    r
}
...
pub unsafe fn push_off() {
    ...
    intr_off();
    if (*(proc::mycpu())).noff == 0 as libc::c_int {
        (*(proc::mycpu())).intena = old
    }
    (*(proc::mycpu())).noff += 1 as libc::c_int;
}
#[no_mangle]
pub unsafe fn pop_off() {
    let mut c: *mut proc::cpu = proc::mycpu();
    if intr_get() != 0 {
        panic(
