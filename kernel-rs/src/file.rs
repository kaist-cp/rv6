...
#[repr(C)]
pub struct devsw {
    pub read: Option<unsafe extern "C" fn(_: libc::c_int, _: uint64,
                                          _: libc::c_int) -> libc::c_int>,
    pub write: Option<unsafe extern "C" fn(_: libc::c_int, _: uint64,
                                           _: libc::c_int) -> libc::c_int>,
}
...
//
#[no_mangle]
pub static mut devsw: [devsw; 10] = [devsw{read: None, write: None,}; 10];
#[no_mangle]
pub static mut ftable: C2RustUnnamed_0 =
...
pub unsafe extern "C" fn fileinit() {
    initlock(&mut ftable.lock,
             b"ftable\x00" as *const u8 as *const libc::c_char as
                 *mut libc::c_char);
}
// file.c
...
pub unsafe extern "C" fn filedup(mut f: *mut file) -> *mut file {
    ...
    if (*f).ref_0 < 1 as libc::c_int {
        panic(b"filedup\x00" as *const u8 as *const libc::c_char as
                  *mut libc::c_char);
    }
    (*f).ref_0 += 1;
    release(&mut ftable.lock);
...
pub unsafe extern "C" fn fileclose(mut f: *mut file) {
    ...
    acquire(&mut ftable.lock);
    if (*f).ref_0 < 1 as libc::c_int {
        panic(b"fileclose\x00" as *const u8 as *const libc::c_char as
                  *mut libc::c_char);
    }
    (*f).ref_0 -= 1;
    if (*f).ref_0 > 0 as libc::c_int { release(&mut ftable.lock); return }
    ff = *f;
    ...
    } else if ff.type_0 as libc::c_uint ==
                  FD_INODE as libc::c_int as libc::c_uint ||
                  ff.type_0 as libc::c_uint ==
                      FD_DEVICE as libc::c_int as libc::c_uint {
        begin_op();
        iput(ff.ip);
        end_op();
    };
...
#[no_mangle]
pub unsafe extern "C" fn filestat(mut f: *mut file, mut addr: uint64)
 -> libc::c_int {
    let mut p: *mut proc_0 = myproc();
    let mut st: stat = stat{dev: 0, ino: 0, type_0: 0, nlink: 0, size: 0,};
...
#[no_mangle]
pub unsafe extern "C" fn fileread(mut f: *mut file, mut addr: uint64,
    ...
    if (*f).type_0 as libc::c_uint == FD_PIPE as libc::c_int as libc::c_uint {
        ...
        r =
            devsw[(*f).major as
                      usize].read.expect("non-null function pointer")(1 as
                                                                          libc::c_int,
                                                                      addr, n)
    } else if (*f).type_0 as libc::c_uint ==
                  FD_INODE as libc::c_int as libc::c_uint {
        ...
        if r > 0 as libc::c_int {
            (*f).off =
                ((*f).off as libc::c_uint).wrapping_add(r as libc::c_uint) as
                    uint as uint
        }
        iunlock((*f).ip);
    } else {
        panic(b"fileread\x00" as *const u8 as *const libc::c_char as
                  *mut libc::c_char);
    }
    return r;
}
...
// addr is a user virtual address.
#[no_mangle]
pub unsafe extern "C" fn filewrite(mut f: *mut file, mut addr: uint64,
                                   mut n: libc::c_int) -> libc::c_int {
    ...
    if (*f).type_0 as libc::c_uint == FD_PIPE as libc::c_int as libc::c_uint {
        ...
        ret =
            devsw[(*f).major as
                      usize].write.expect("non-null function pointer")(1 as
                                                                           libc::c_int,
                                                                       addr,
                                                                       n)
    } else if (*f).type_0 as libc::c_uint ==
    ...
    // might be writing a device like the console.
        let mut max: libc::c_int =
            (MAXOPBLOCKS - 1 as libc::c_int - 1 as libc::c_int -
                 2 as libc::c_int) / 2 as libc::c_int * BSIZE;
        ...
        while i < n {
            ...
            ilock((*f).ip);
            r =
                writei((*f).ip, 1 as libc::c_int,
                       addr.wrapping_add(i as libc::c_ulong), (*f).off,
                       n1 as uint);
            if r > 0 as libc::c_int {
                (*f).off =
                    ((*f).off as libc::c_uint).wrapping_add(r as libc::c_uint)
                        as uint as uint
            }
            iunlock((*f).ip);
            end_op();
            ...
            if r != n1 {
                panic(b"short filewrite\x00" as *const u8 as
        ...
        panic(b"filewrite\x00" as *const u8 as *const libc::c_char as
                  *mut libc::c_char);
    }
    return ret;
}
