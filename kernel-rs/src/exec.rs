...
// exec.c
#[no_mangle]
pub unsafe extern "C" fn exec(mut path: *mut libc::c_char,
 ...
 -> libc::c_int {
    ...
    let mut oldpagetable: pagetable_t = 0 as *mut uint64;
    let mut p: *mut proc_0 = myproc();
    begin_op();
    ip = namei(path);
    if ip.is_null() { end_op(); return -(1 as libc::c_int) }
    ilock(ip);
    // Check ELF header
    if !(readi(ip, 0 as libc::c_int, &mut elf as *mut elfhdr as uint64,
               0 as libc::c_int as uint,
               ::core::mem::size_of::<elfhdr>() as libc::c_ulong as uint) as
             libc::c_ulong !=
        ...
        if !(elf.magic != ELF_MAGIC) {
            ...
            if !pagetable.is_null() {
                ...
                loop  {
                    ...
                    if readi(ip, 0 as libc::c_int,
                             &mut ph as *mut proghdr as uint64, off as uint,
                             ::core::mem::size_of::<proghdr>() as
                                 libc::c_ulong as uint) as libc::c_ulong !=
                    ...
                    if !(ph.type_0 != ELF_PROG_LOAD as libc::c_uint) {
                        ...
                        sz =
                            uvmalloc(pagetable, sz,
                                     ph.vaddr.wrapping_add(ph.memsz));
                        ...
                        if loadseg(pagetable, ph.vaddr, ip, ph.off as uint,
                                   ph.filesz as uint) < 0 as libc::c_int {
                            current_block = 7080392026674647309;
                            break ;
                        }
                    }
                    ...
                    off =
                        (off as
                             libc::c_ulong).wrapping_add(::core::mem::size_of::<proghdr>()
                                                             as libc::c_ulong)
  ...
  // Use the second as the user stack.
                        ...
                        sz =
                            uvmalloc(pagetable, sz,
                                     sz.wrapping_add((2 as libc::c_int *
                                                          PGSIZE) as
                                                         libc::c_ulong));
                        if !(sz == 0 as libc::c_int as libc::c_ulong) {
                            uvmclear(pagetable,
                                     sz.wrapping_sub((2 as libc::c_int *
                                                          PGSIZE) as
                                                         libc::c_ulong));
                            sp = sz;
                            stackbase =
                                sp.wrapping_sub(PGSIZE as libc::c_ulong);
                            // Push argument strings, prepare rest of stack in ustack.
                            argc =
                                0 as libc::c_int as
                                    uint64; // riscv sp must be 16-byte aligned
                            loop  {
                                ...
                                sp =
                                    (sp as
                                         libc::c_ulong).wrapping_sub((strlen(*argv.offset(argc
                                                                                              as
                                                                                              isize))
                                                                          +
                                                                          1 as
                                ...
                                sp =
                                    (sp as
                                         libc::c_ulong).wrapping_sub(sp.wrapping_rem(16
                                                                                         as
                                                                                         libc::c_int
                                                                                         as
                                                                                         libc::c_ulong))
                                ...
                                if copyout(pagetable, sp,
                                           *argv.offset(argc as isize),
                                           (strlen(*argv.offset(argc as
                                                                    isize)) +
                            ...
                            match current_block {
                                7080392026674647309 => { }
                                _ => {
                                    ustack[argc as usize] =
                                        0 as libc::c_int as uint64;
                                    // push the array of argv[] pointers.
                                    sp =
                                        (sp as
                                             libc::c_ulong).wrapping_sub(argc.wrapping_add(1
                                                                                               as
                                                                                               libc::c_int
                                                                                               as
                                                                                               libc::c_ulong).wrapping_mul(::core::mem::size_of::<uint64>()
                                                                                                                               as
                                                                                                                               libc::c_ulong))
                                            as uint64 as uint64;
                                    sp =
                                    ...
                                    if !(sp < stackbase) {
                                        if !(copyout(pagetable, sp,
                                                     ustack.as_mut_ptr() as
                                                         *mut libc::c_char,
                                                     argc.wrapping_add(1 as
                                                                           libc::c_int
                                                                           as
                                                                           libc::c_ulong).wrapping_mul(::core::mem::size_of::<uint64>()
                                                                                                           as
  ...
  // value, which goes in a0.
                                            ...
                                            oldpagetable =
                                                (*p).pagetable; // initial program counter = main
                                            (*p).pagetable =
                                                pagetable; // initial stack pointer
                                            (*p).sz =
                                                sz; // this ends up in a0, the first argument to main(argc, argv)
                                            (*(*p).tf).epc = elf.entry;
                                            (*(*p).tf).sp = sp;
    ...
    if !pagetable.is_null() { proc_freepagetable(pagetable, sz); }
    if !ip.is_null() { iunlockput(ip); end_op(); }
...
unsafe extern "C" fn loadseg(mut pagetable: pagetable_t, mut va: uint64,
                             mut ip: *mut inode, mut offset: uint,
                             mut sz: uint) -> libc::c_int {
    let mut i: uint = 0;
    let mut n: uint = 0;
    let mut pa: uint64 = 0;
    if va.wrapping_rem(PGSIZE as libc::c_ulong) !=
           0 as libc::c_int as libc::c_ulong {
        panic(b"loadseg: va must be page aligned\x00" as *const u8 as
    ...
    while i < sz {
        ...
        if pa == 0 as libc::c_int as libc::c_ulong {
            panic(b"loadseg: address should exist\x00" as *const u8 as
                      *const libc::c_char as *mut libc::c_char);
        }
        if sz.wrapping_sub(i) < PGSIZE as libc::c_uint {
            n = sz.wrapping_sub(i)
        } else { n = PGSIZE as uint }
        if readi(ip, 0 as libc::c_int, pa, offset.wrapping_add(i), n) as
               libc::c_uint != n {
            return -(1 as libc::c_int)
        }
        i =
            (i as libc::c_uint).wrapping_add(PGSIZE as libc::c_uint) as uint
                as uint
    }
