...
// Edit index
#[no_mangle]
pub static mut cons: C2RustUnnamed_0 =
...
//
#[no_mangle]
pub unsafe extern "C" fn consolewrite(mut user_src: libc::c_int,
                                      mut src: uint64, mut n: libc::c_int)
 -> libc::c_int {
    ...
    while i < n {
        ...
        if either_copyin(&mut c as *mut libc::c_char as *mut libc::c_void,
                         user_src, src.wrapping_add(i as libc::c_ulong),
...
//
#[no_mangle]
pub unsafe extern "C" fn consoleread(mut user_dst: libc::c_int,
                                     mut dst: uint64, mut n: libc::c_int)
 -> libc::c_int {
    ...
    // input into cons.buffer.
        while cons.r == cons.w {
            ...
            sleep(&mut cons.r as *mut uint as *mut libc::c_void,
                  &mut cons.lock);
        }
        let fresh0 = cons.r;
        cons.r = cons.r.wrapping_add(1);
        c =
            cons.buf[fresh0.wrapping_rem(INPUT_BUF as libc::c_uint) as usize]
                as libc::c_int;
        ...
        // caller gets a 0-byte result.
            ...
            if either_copyout(user_dst, dst,
                              &mut cbuf as *mut libc::c_char as
                                  *mut libc::c_void,
...
pub unsafe extern "C" fn consoleintr(mut c: libc::c_int) {
    ...
    match c {
        ...
        21 => {
            // Kill line.
            while cons.e != cons.w &&
                      cons.buf[cons.e.wrapping_sub(1 as libc::c_int as
                                                       libc::c_uint).wrapping_rem(INPUT_BUF
                                                                                      as
                                                                                      libc::c_uint)
        ...
        _ => {
            if c != 0 as libc::c_int &&
                   cons.e.wrapping_sub(cons.r) < INPUT_BUF as libc::c_uint {
                c = if c == '\r' as i32 { '\n' as i32 } else { c };
                ...
                cons.buf[fresh1.wrapping_rem(INPUT_BUF as libc::c_uint) as
                             usize] = c as libc::c_char;
                if c == '\n' as i32 || c == 'D' as i32 - '@' as i32 ||
                       cons.e ==
                           cons.r.wrapping_add(INPUT_BUF as libc::c_uint) {
...
pub unsafe extern "C" fn consoleinit() {
    initlock(&mut cons.lock,
             b"cons\x00" as *const u8 as *const libc::c_char as
                 *mut libc::c_char);
    uartinit();
