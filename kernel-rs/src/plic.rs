...
pub const PLIC_PENDING: libc::c_long =
    PLIC + 0x1000 as libc::c_int as libc::c_long;
// plic.c
//
...
pub unsafe extern "C" fn plicinit() {
    ...
    *((PLIC + (VIRTIO0_IRQ * 4 as libc::c_int) as libc::c_long) as
          *mut uint32) = 1 as libc::c_int as uint32;
}
#[no_mangle]
pub unsafe extern "C" fn plicinithart() {
    let mut hart: libc::c_int = cpuid();
    // set uart's enable bit for this hart's S-mode. 
    *((PLIC + 0x2080 as libc::c_int as libc::c_long +
           (hart * 0x100 as libc::c_int) as libc::c_long) as *mut uint32) =
        ((1 as libc::c_int) << UART0_IRQ | (1 as libc::c_int) << VIRTIO0_IRQ)
            as uint32;
    // set this hart's S-mode priority threshold to 0.
    *((PLIC + 0x201000 as libc::c_int as libc::c_long +
...
pub unsafe extern "C" fn plic_claim() -> libc::c_int {
    ...
    let mut irq: libc::c_int =
        *((PLIC + 0x201004 as libc::c_int as libc::c_long +
               (hart * 0x2000 as libc::c_int) as libc::c_long) as *mut uint32)
            as libc::c_int;
    return irq;
}
