...
extern "C" {
    ...
    fn copyin(_: pagetable_t, _: *mut libc::c_char, _: uint64, _: uint64)
     -> libc::c_int;
    #[no_mangle]
    fn copyinstr(_: pagetable_t, _: *mut libc::c_char, _: uint64, _: uint64)
     -> libc::c_int;
    #[no_mangle]
    fn sys_chdir() -> uint64;
    #[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn fetchaddr(mut addr: uint64, mut ip: *mut uint64)
 -> libc::c_int {
    let mut p: *mut proc_0 = myproc();
    if addr >= (*p).sz ||
           addr.wrapping_add(::core::mem::size_of::<uint64>() as
                                 libc::c_ulong) > (*p).sz {
        return -(1 as libc::c_int)
    }
    if copyin((*p).pagetable, ip as *mut libc::c_char, addr,
...
#[no_mangle]
pub unsafe extern "C" fn fetchstr(mut addr: uint64,
                                  mut buf: *mut libc::c_char,
                                  mut max: libc::c_int) -> libc::c_int {
    let mut p: *mut proc_0 = myproc();
    let mut err: libc::c_int =
        copyinstr((*p).pagetable, buf, addr, max as uint64);
...
unsafe extern "C" fn argraw(mut n: libc::c_int) -> uint64 {
    ...
    panic(b"argraw\x00" as *const u8 as *const libc::c_char as
              *mut libc::c_char);
}
// syscall.c
// Fetch the nth 32-bit system call argument.
#[no_mangle]
pub unsafe extern "C" fn argint(mut n: libc::c_int, mut ip: *mut libc::c_int)
 -> libc::c_int {
...
#[no_mangle]
pub unsafe extern "C" fn argaddr(mut n: libc::c_int, mut ip: *mut uint64)
 -> libc::c_int {
    *ip = argraw(n);
    return 0 as libc::c_int;
}
...
#[no_mangle]
pub unsafe extern "C" fn argstr(mut n: libc::c_int,
                                mut buf: *mut libc::c_char,
                                mut max: libc::c_int) -> libc::c_int {
    let mut addr: uint64 = 0;
    if argaddr(n, &mut addr) < 0 as libc::c_int { return -(1 as libc::c_int) }
    return fetchstr(addr, buf, max);
}
static mut syscalls: [Option<unsafe extern "C" fn() -> uint64>; 22] =
    unsafe {
        [None, Some(sys_fork as unsafe extern "C" fn() -> uint64),
    ...
    if num > 0 as libc::c_int &&
           (num as libc::c_ulong) <
               (::core::mem::size_of::<[Option<unsafe extern "C" fn()
                                                   -> uint64>; 22]>() as
                    libc::c_ulong).wrapping_div(::core::mem::size_of::<Option<unsafe extern "C" fn()
                                                                                  ->
                                                                                      uint64>>()
                                                    as libc::c_ulong) &&
           syscalls[num as usize].is_some() {
        (*(*p).tf).a0 =
            syscalls[num as usize].expect("non-null function pointer")()
    } else {
        printf(b"%d %s: unknown sys call %d\n\x00" as *const u8 as
                   *const libc::c_char as *mut libc::c_char, (*p).pid,
               (*p).name.as_mut_ptr(), num);
