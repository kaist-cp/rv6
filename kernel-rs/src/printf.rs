...
unsafe extern "C" fn printint(mut xx: libc::c_int, mut base: libc::c_int,
                              mut sign: libc::c_int) {
    let mut buf: [libc::c_char; 16] = [0; 16];
    let mut i: libc::c_int = 0;
    let mut x: uint = 0;
    if sign != 0 &&
           { sign = (xx < 0 as libc::c_int) as libc::c_int; (sign) != 0 } {
        x = -xx as uint
    } else { x = xx as uint }
    i = 0 as libc::c_int;
    loop  {
        ...
        i = i + 1;
        buf[fresh0 as usize] =
            digits[x.wrapping_rem(base as libc::c_uint) as usize];
        x =
            (x as libc::c_uint).wrapping_div(base as libc::c_uint) as uint as
                uint;
        if !(x != 0 as libc::c_int as libc::c_uint) { break ; }
    }
    if sign != 0 {
        let fresh1 = i;
        i = i + 1;
...
pub unsafe extern "C" fn printf(mut fmt: *mut libc::c_char, mut args: ...) {
    ...
    locking = pr.locking;
    if locking != 0 { acquire(&mut pr.lock); }
    if fmt.is_null() {
        panic(b"null fmt\x00" as *const u8 as *const libc::c_char as
                  *mut libc::c_char);
    }
    ap = args.clone();
    i = 0 as libc::c_int;
    loop  {
        ...
        if c != '%' as i32 {
            ...
            match c {
                100 => {
                    printint(ap.as_va_list().arg::<libc::c_int>(),
                ...
                112 => { printptr(ap.as_va_list().arg::<uint64>()); }
                115 => {
    ...
    if locking != 0 { release(&mut pr.lock); };
}
#[no_mangle]
pub unsafe extern "C" fn panic(mut s: *mut libc::c_char) -> ! {
    pr.locking = 0 as libc::c_int;
    printf(b"panic: \x00" as *const u8 as *const libc::c_char as
               *mut libc::c_char);
    printf(s);
    printf(b"\n\x00" as *const u8 as *const libc::c_char as
               *mut libc::c_char);
    ::core::ptr::write_volatile(&mut panicked as *mut libc::c_int,
                                1 as libc::c_int);
    loop  { };
}
#[no_mangle]
pub unsafe extern "C" fn printfinit() {
    initlock(&mut pr.lock,
             b"pr\x00" as *const u8 as *const libc::c_char as
                 *mut libc::c_char);
