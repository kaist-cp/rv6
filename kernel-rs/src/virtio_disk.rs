...
pub unsafe extern "C" fn virtio_disk_init() {
    ...
    initlock(
        ...
        b"virtio_disk\x00" as *const u8 as *mut u8,
    );
    ...
    memset(
        ...
        0 as i32,
        ::core::mem::size_of::<[u8; 8192]>() as u64 as u32,
    );
    ...
    disk.0.avail = (disk.0.desc as *mut u8)
    ...
    let mut i: i32 = 0;
    while i < NUM {
        disk.0.free[i as usize] = 1 as i32 as u8;
        i += 1
    }
...
unsafe extern "C" fn alloc_desc() -> i32 {
    ...
    while i < NUM {
        if disk.0.free[i as usize] != 0 {
            disk.0.free[i as usize] = 0 as i32 as u8;
            return i;
        }
        i += 1
    }
...
unsafe extern "C" fn free_desc(mut i: i32) {
    ...
    disk.0.free[i as usize] = 1 as i32 as u8;
    wakeup(
        &mut *disk.0.free.as_mut_ptr().offset(0 as i32 as isize) as *mut u8 as *mut libc::c_void,
    );
}
...
pub unsafe extern "C" fn virtio_disk_rw(mut b: *mut Buf, mut write: i32) {
    ...
    while alloc3_desc(idx.as_mut_ptr()) != 0 as i32 {
        sleep(
            &mut *disk.0.free.as_mut_ptr().offset(0 as i32 as isize) as *mut u8
                as *mut libc::c_void,
            &mut disk.0.vdisk_lock,
        );
    }
