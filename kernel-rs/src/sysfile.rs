...
impl RcFile<'static> {
    ...
    fn fdalloc(self, proc_data: &mut ProcData) -> Result<i32, Self> {
        for fd in 0..NOFILE {
...
{
    let (ptr, name) = path.nameiparent(proc_data)?;
    let mut dp = ptr.lock();
...
impl Kernel {
    ...
    fn link(&self, oldname: &CStr, newname: &CStr, proc_data: &ProcData) -> Result<(), ()> {
        let tx = self.file_system.begin_transaction();
        let ptr = Path::new(oldname).namei(proc_data)?;
        let mut ip = ptr.lock();
        ...
        if let Ok((ptr2, name)) = Path::new(newname).nameiparent(proc_data) {
    ...
    /// Returns Ok(()) on success, Err(()) on error.
    fn unlink(&self, filename: &CStr, proc_data: &ProcData) -> Result<(), ()> {
        let de: Dirent = Default::default();
        let tx = self.file_system.begin_transaction();
        let (ptr, name) = Path::new(filename).nameiparent(proc_data)?;
        let mut dp = ptr.lock();
    ...
    fn open(
        ...
        let (ip, typ) = if omode.contains(FcntlFlags::O_CREATE) {
            create(name, InodeType::File, &tx, proc_data, |ip| {
                ip.deref_inner().typ
            })?
        } else {
            let ptr = name.namei(proc_data)?;
            let ip = ptr.lock();
        ...
        let fd = f.fdalloc(proc_data).map_err(|_| ())?;
        Ok(fd as usize)
    }

    /// Create a new directory.
    /// Returns Ok(()) on success, Err(()) on error.
    fn mkdir(&self, dirname: &CStr, proc_data: &ProcData) -> Result<(), ()> {
        let tx = self.file_system.begin_transaction();
        create(Path::new(dirname), InodeType::Dir, &tx, proc_data, |_| ())?;
        Ok(())
    }
    ...
    /// Returns Ok(()) on success, Err(()) on error.
    fn chdir(&self, dirname: &CStr, proc_data: &mut ProcData) -> Result<(), ()> {
        ...
        let ptr = Path::new(dirname).namei(proc_data)?;
        let ip = ptr.lock();
        if ip.deref_inner().typ != InodeType::Dir {
            return Err(());
        }
        drop(ip);
        proc_data.cwd = Some(ptr);
        Ok(())
    }

    /// Create a pipe, put read/write file descriptors in fd0 and fd1.
    /// Returns Ok(()) on success, Err(()) on error.
    fn pipe(&self, fdarray: UVAddr, proc_data: &mut ProcData) -> Result<(), ()> {
        let (pipereader, pipewriter) = AllocatedPipe::alloc()?;

        let mut fd0 = pipereader.fdalloc(proc_data).map_err(|_| ())?;
...
impl Kernel {
    ...
    pub unsafe fn sys_dup(&self, proc: &CurrentProc<'_>) -> Result<usize, ()> {
        ...
        let newfile = f.clone();
        let fd = newfile.fdalloc(proc.deref_mut_data()).map_err(|_| ())?;
        Ok(fd as usize)
    }
    ...
    pub unsafe fn sys_fstat(&self, proc: &CurrentProc<'_>) -> Result<usize, ()> {
        ...
        unsafe { f.stat(UVAddr::new(st), proc.deref_mut_data())? };
        Ok(0)
    }
    ...
    pub unsafe fn sys_link(&self, proc: &CurrentProc<'_>) -> Result<usize, ()> {
        ...
        let old = unsafe { argstr(0, &mut old, proc.deref_mut_data())? };
        let new = unsafe { argstr(1, &mut new, proc.deref_mut_data())? };
    ...
    pub unsafe fn sys_unlink(&self, proc: &CurrentProc<'_>) -> Result<usize, ()> {
        ...
        let path = unsafe { argstr(0, &mut path, proc.deref_mut_data())? };
    ...
    pub unsafe fn sys_open(&'static self, proc: &CurrentProc<'_>) -> Result<usize, ()> {
        ...
        self.open(path, omode, proc.deref_mut_data())
    }
    ...
    pub unsafe fn sys_chdir(&self, proc: &CurrentProc<'_>) -> Result<usize, ()> {
        let mut path: [u8; MAXPATH] = [0; MAXPATH];
        let path = unsafe { argstr(0, &mut path, proc.deref_mut_data())? };
        self.chdir(path, proc.deref_mut_data())?;
        Ok(0)
    }
    ...
    pub unsafe fn sys_exec(&self, proc: &CurrentProc<'_>) -> Result<usize, ()> {
        ...
        for i in 0..MAXARG {
            ...
            if unsafe { fetchstr(UVAddr::new(uarg), &mut page[..], proc.deref_mut_data()) }.is_err()
            {
                self.free(page);
                break;
            }
    ...
    pub fn sys_pipe(&self, proc: &CurrentProc<'_>) -> Result<usize, ()> {
        ...
        self.pipe(fdarray, proc.deref_mut_data())?;
        Ok(0)
    }
}
