...
use crate::arch::{
    asm::r_tp,
    memlayout::{plic_sclaim, plic_senable, plic_spriority, MemLayoutImpl, PLIC},
};
use crate::memlayout::MemLayout;

pub unsafe fn intr_init() {
    // set desired IRQ priorities non-zero (otherwise disabled).
    unsafe { *((PLIC.wrapping_add(MemLayoutImpl::UART0_IRQ.wrapping_mul(4))) as *mut u32) = 1 };
    unsafe { *((PLIC + MemLayoutImpl::VIRTIO0_IRQ * 4) as *mut u32) = 1 };
}
...
pub unsafe fn intr_init_core() {
    ...
    unsafe {
        *(plic_senable(hart) as *mut u32) =
            (1 << MemLayoutImpl::UART0_IRQ | 1 << MemLayoutImpl::VIRTIO0_IRQ) as u32
    };
